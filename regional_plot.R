...
tiff(paste(args[4],".tiff",sep=""),2*2**10,2*2**10,pointsize=10,res=300,compression="lzw")
genemap <- args[7]
genetic_map <- args[8]
...
SNP.name<-ld[1,3]
names(ld)[6]<-"SNP"
data<-read.table(args[4],header=F)
names(data)[1]<-"SNP"
...
CHR<-args[5]
CM.Mb<-read.table(paste(genetic_map,"genetic_map_chr",CHR,"_combined_b37.txt",sep=""),header=F,skip=1)
names(CM.Mb)<-c("V3","CM.Mb","CM")

#generate plot in two steps; 1/ SNPs not in LD with top SNP, 2/ SNPs in LD with top SNP
plot(frame=F,data_nor2$V3,-log10(data_nor2$V10),bg=ifelse(data_nor2$R2>0.8,cols[1],ifelse(data_nor2$R2>0.6,cols[2],ifelse(data_nor2$R2>0.4,cols[3],ifelse(data_nor2$R2>0.2,cols[4],"grey")))),pch=21, col=ifelse(data_nor2$R2>0.2,"black","grey"),ylab="",xlab="",ylim=c(0,ymax),xlim=c(BP_min,BP_max),xaxt='n',yaxt='n')
par(new=T)
plot(frame=F,data_r2$V3,-log10(data_r2$V10),
...
genemap<-read.table(genemap,header=F,colClasses=c("character","integer","integer","character"))
